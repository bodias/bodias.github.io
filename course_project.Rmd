...
The dataset used in this project is available at [github](https://github.com/bodias/datasciencecoursera/tree/master/practical_machine_learning/project_course)
...
The selected method, Random Forests, was applied to the training set using the standardized dataset with the PCA variables. The train control was also set to use Cross Validation with 5 folds.
...
The training using Random Forests with Cross Validation took `r round(run_time,2)` minutes to run.
...
The In-Sample Error Rate is 0.02 or 2%

### Predicting the results on the Test Set

Then the trained model was applied to the test data set to predict the "classe" of each observation. To do so, I have to Standardize and run PCA used the models built from the training data. 

Because the actual "classe" is not provided for the Test Set, it is only possible to estimate that the Out of Sample error rate will be greater than the In-Sample Error rate, which is 2%
